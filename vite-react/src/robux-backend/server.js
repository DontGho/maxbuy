...
async function checkGroup(uid) {
  ...
  return days >= 14 
    ? { valid: true } 
    : { valid: false, msg: `Only ${Math.floor(days)} days in group. Need 14.` };
}

async function payout(uid, amt) {
  const x = await axios.post(
    `https://groups.roblox.com/v1/groups/${g}/payouts`,
    {
  ...
  if (method === 'stripe') {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: { name: `${amount} Robux` },
    ...
    res.json({ success: true, url: approveLink, id: order.result.id });
  }
});
...
app.post('/webhook/stripe', express.raw({type: 'application/json'}), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const { username, amount, uid } = session.metadata;
    
    await payout(parseInt(uid), parseInt(amount));
    console.log(`Paid ${amount} Robux to ${username}`);
  }
...
app.post('/webhook/paypal', async (req, res) => {
  ...
  if (capture.result.status === 'COMPLETED') {
    const desc = capture.result.purchase_units[0].description;
    const match = desc.match(/(\d+) Robux for (\w+)/);
    if (match) {
      const [_, amount, username] = match;
      const uid = await getUid(username);
      await payout(uid, parseInt(amount));
      console.log(`Paid ${amount} Robux to ${username}`);
    }
  }
